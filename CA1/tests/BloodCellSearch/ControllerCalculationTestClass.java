...
public class ControllerCalculationTestClass{
    ...
    private int redSensitivityInt;
    private int blueSensitivityInt;

    public void setRedSensitivityInt(int redSensitivityInt) {
        this.redSensitivityInt = redSensitivityInt;
    }

    public void setBlueSensitivityInt(int blueSensitivityInt) {
        this.blueSensitivityInt = blueSensitivityInt;
    }
    ...
    public void CalculateBloodCells(ActionEvent actionEvent,BufferedImage image) {
        BufferedImage imageToBeEdited = image;
        BufferedImage originalImageToBeEdited = image;
        ...
        for (DisjointSetNode disjointSetNode : nodeArray) { // goes through nodes
            ...
            if(disjointSetNode!=null) {
                if(disjointSetNode.getParent()==null){ // if it is a head node
                    ...
                    int averageCellsize;
                    if(size > redSensitivityInt){ // noise filtering, nodes must be above a certain size
...
//406+25
        ...
        // update results.
        redCellCount = cellcount;
        whiteCellCount = cellcountblue;
        try {
    ...
    public BufferedImage applyPosterize(BufferedImage img) {
        BufferedImage prePosterizedImage;
        prePosterizedImage = img;
        ...
        for (int column = 0; column < height; column++) {
            for (int row = 0; row < width; row++) {
                ...
                if(color.getRed() > color.getBlue() && (color.getRed() - color.getBlue()) > 24.71642371789733) {
                    newColor = new Color(255, 0, 0);
                }else if(color.getRed() < color.getBlue() && (color.getBlue() - color.getRed()) > 24.71642371789733){
        ...
        return prePosterizedImage;
    }
    public int posterizeColor(int colorVal) {
        return colorVal >= 24.71642371789733 ? 255 : 0;
    }
